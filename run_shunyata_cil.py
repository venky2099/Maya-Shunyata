...
def run_shunyata_cil(seed: int = 42, karma_threshold: float = KARMA_THRESHOLD):
    print("MayaNexusVS2026NLL_Bengaluru_Narasimha")
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device} | Seed: {seed} | Karma threshold: {karma_threshold}")
    ...
    # weight snapshot for Karma delta computation
    ...
    for task_id in range(NUM_TASKS):
        ...
        if task_id > 0:
            ...
            # Shunyata pruning at task boundary
        ...
        for epoch in range(EPOCHS_PER_TASK):
            ...
            for batch_idx, (images, labels) in enumerate(tqdm(
                ...
                # Karma accumulation + mask enforcement
                with torch.no_grad():
    ...
    print(f"\n{'='*50}")
    print(f"  Maya-Shunyata CIL | seed={seed} | threshold={karma_threshold}")
    print(f"  AA  : {final['AA']}%")
